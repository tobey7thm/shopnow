...
$pagesize=8; //设定每一页显示的记录数
    ...
    $rs = mysql_query( "select * from goods where owner='$_SESSION[user_name]' and delstates=1 order by addtime DESC"); //这里有第二个可选参数，指定打开的连接
//-----------------------------------------------------------------------------------------------//
//分页逻辑处理
//-----------------------------------------------------------------------------------------------
...
//↓读取指定记录数
$rs=mysql_query("select * from goods where owner='$_SESSION[user_name]'  and delstates=1 limit $offset,$pagesize");//取得—当前页—记录集！
...
?> 
       ...
       <div class="page_content_offset">
        <div class="container">
          ...
          <h2 class="tt_uppercase color_dark m_bottom_25"><?php echo $_SESSION['user_name']?>发布的待审核商品</h2>
          <div class="row clearfix">
          
<?php              
                     
          $sql="select * from goods where owner='$_SESSION[user_name]' and delstates=1 order by addtime DESC limit $offset,$pagesize";
              ...
              <figure class="t_align_c">
                ...
                <figcaption>
                  ...
                  <ul class="clearfix horizontal_list social_icons d_inline_b t_md_align_c">
                    <a href=javascript:del('<?php echo $data->id;?>')><button class="button_type_4 bg_scheme_color r_corners tr_all_hover color_light f_left mw_0">删除</button></a>
                  </ul>
                </figcaption> 
              </figure>
            </div>
