<?php
session_start();
 require("./include/connect_db.php");
if($_SESSION['user_name']=="")
{
	echo"<script>alert('请先登录!');history.back(-1);</script>";
	exit;
}
$sql1="select id from user where username = '$_SESSION[user_name]'";
	$result1=Connect_db($sql1);

	$data1=mysql_fetch_object($result1);
	$uid=$data1->id;

$sql2="delete from collect where uid='$uid' and pid='$id'";

$result2=Connect_db($sql2);
if($result2)
{
	echo"<script>alert('删除成功！');history.back(-1);</script>";
}
else{
	echo"<script>alert('已经删除该商品！');history.back(-1);</script>";
}

?>