...
$where2="";
if($pricef!="") $where2.=" and price between '$pricef' and '$pricel'";
$where3="";
if($viewf!="") $where3.=" and hitsnum between '$viewf' and '$viewl'";

    $rs = mysql_query( "select * from goods where delstates=0 and ud=0 $where $where2 $where3"); //这里有第二个可选参数，指定打开的连接
//-----------------------------------------------------------------------------------------------//
//分页逻辑处理
//-----------------------------------------------------------------------------------------------
...
//↓读取指定记录数
$rs=mysql_query("select * from goods where delstates=0 and ud=0 $where $where2 $where3 limit $offset,$pagesize");//取得—当前页—记录集！
...
<section class="products_container list_type clearfix m_bottom_5 m_left_0 m_right_0">
  ...
  $where3="";
  if($viewf!="") $where3.=" and hitsnum between '$viewf' and '$viewl'";
  $sql="select * from goods where delstates=0 and ud=0 $where $where2 $where3 order by addtime DESC limit $offset,$pagesize";
        ...
        <figcaption>
          <div class="clearfix">
           ...
           <div class="f_right f_sm_none t_align_r t_sm_align_l">
            ...
            <a href="collect_add.php?id=<?php echo $data->id;?>"><button class="button_type_4 bg_light_color_2 d_sm_inline_middle f_sm_none m_sm_left_5 tr_all_hover f_right r_corners color_dark mw_0 p_hr_0"><i class="fa fa-heart-o"></i></button></a>
          </div>
        </div>
      </figcaption>
    </figure>
...
//取顶级分类
 ...
 <figure class="widget shadow r_corners wrapper m_bottom_30">
  ...
  <div class="widget_content">
    <!--filter form-->
    <form name="form1" method="get" onsubmit="return isok(this)" action="?pricef=<?php echo $pricef?>&pricel=<?php echo $pricel?>&viewf=<?php echo $viewf?>&viewl=<?php echo $viewl?>">
     <fieldset class="m_bottom_25">
      ...
      <div class="clearfix">
        <input type="text" name="pricef" class="r_corners f_left type_2">
        <input type="text" name="pricel" class="r_corners f_left type_2 m_left_10">
      </div>
    </fieldset>
    <fieldset class="m_bottom_25">
      <legend class="default_t_color f_size_large m_bottom_15 clearfix full_width relative">
        <b class="f_left">浏览量</b>
        <button type="button" class="f_size_medium f_right color_dark bg_tr tr_all_hover close_fieldset"><i class="fa fa-times lh_inherit"></i></button>
      </legend>
      <div class="clearfix">
        <input type="text" name="viewf" class="r_corners f_left type_2">
        <input type="text" name="viewl" class="r_corners f_left type_2 m_left_10">
      </div>
    </fieldset>
...
{
  if (theform.pricef.value>theform.pricel.value)
  {
    alert("输入有误！");
    theform.pricef.focus();
    return (false);
  }
  if (theform.viewf.value>theform.viewl.value)
  {
    alert("输入有误！");
    theform.viewf.focus();
    return (false);
  ...
  if (theform.pricef.value<0)
  {
    alert("价格不能为负数！");
    theform.pricef.focus();
    return (false);
  }
  if (theform.viewf.value<0)
  {
    alert("浏览量不能为负数！");
    theform.viewf.focus();
    return (false);
  }
  if (theform.pricel.value<0)
  {
    alert("价格不能为负数！");
    theform.pricel.focus();
    return (false);
  }
  if (theform.viewl.value<0)
  {
    alert("浏览量不能为负数！");
    theform.viewl.focus();
    return (false);
